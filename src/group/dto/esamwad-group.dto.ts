...
export class EsamwadDto {
  ...
  constructor(obj: any) {
    this.groupId = obj?.id ? `${obj.id}` : "";

    this.schoolId = obj?.school_grades[0].school_id
      ? `${obj.school_grades[0].school_id}`
      : "";
    this.name = obj?.number ? `${obj.number}` : "";
